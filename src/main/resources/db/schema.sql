...
CREATE TABLE IF NOT EXISTS message_feedback (
    ...
    message_id      VARCHAR(64) NOT NULL,       -- 前端消息 id（assistant 消息）
    question        TEXT,                        -- 用户提问原文（方便离线分析）
    answer          TEXT,                        -- 回答原文
    feedback        VARCHAR(16) NOT NULL,        -- 'thumbs_up' / 'thumbs_down'
    comment         TEXT,                        -- 用户可选评论文本
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (session_id, message_id)              -- 每条消息只允许一条反馈
);
CREATE INDEX IF NOT EXISTS idx_feedback_session ON message_feedback (session_id);
CREATE INDEX IF NOT EXISTS idx_feedback_type ON message_feedback (feedback);
